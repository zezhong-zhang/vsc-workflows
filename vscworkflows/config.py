...
"""
Set of scripts for configuring the workflows.
...
def fworker(fireworker_file=None, fworker_name="base"):
    ...
    try:
        os.makedirs(
            os.path.join(os.path.expanduser("~"), ".workflow_config", "fworker")
        )
    except FileExistsError:
        pass
...
def jobscript(template_file, fworker_name="base"):
    ...
    Args:
        template_file (str): job_template.sh file to be tied to the fworker
            configuration.
        fworker_name (str): Which fworker configuration of the workflows to add the job
            template to. Note that the corresponding queue adapter must contain the variables
    ...
    config_template_file = os.path.join(os.path.expanduser("~"), ".workflow_config",
...
def check():
    """
    Check the workflow configuration files and print out the configurations present.

    """
    config_dir = os.path.join(os.path.expanduser("~"), ".workflow_config")

    if not os.path.exists(config_dir):
        print("\nNo configuration directory found! Use 'vsc config' to set up the "
              "workflow configuration.")
        return

    if not os.path.exists(os.path.join(config_dir, "launchpad")):
        print("\nNo launchpad directory found. Use 'vsc config launchpad' to set up a "
    ...
    if not os.path.exists(os.path.join(config_dir, "fworker")):
        print("\nNo fworker directory found. Use 'vsc config fworker' to set up a "
